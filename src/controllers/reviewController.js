...
const createReview = async function (req, res) {
  try {
    ...
    if (!validator.isValidObjectId(bookId)){ return res.status(400).send({ status: false, message: "this is not a valid book Id" });
    }

    let findBook = await bookModel.findOne({ _id: bookId, isDeleted: false });
    if (!validator.isValid(findBook)){return res.status(404).send({ status: false, message: "no books with this Books id" });
    }

    if (findBook.isDeleted === true){return res.status(404).send({ status: false, message: "This book has been deleted" });
    }

    if (!rating) { return res.status(400).send({ status: false, message: "rating is a required field" });
    }
    ...
    if (reviewedBy) {if (!validator.isValid(reviewedBy)) {
      return res.status(400).send({ status: false, message: "reviewBy is a required field" });
      }
    }
    ...
    const reviewList = await reviewModel .findOne({ _id: reviewCreated._id }) .select({ isDeleted: 0, createdAt: 0, updatedAt: 0, __v: 0 });
